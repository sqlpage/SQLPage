...
type Point = { x: XValue; y: number | string | null; z?: number };
...
test("align_series counts an x a series skipped as zero (#727)", () => {
...
test("align_series counts a null value as a value the series never measured", () => {
  const [, b] = align_series([
    series("a", { x: "Q1", y: 1 }),
    series("b", { x: "Q1", y: null }),
  ]);

  assert.deepEqual(b.data, [{ x: "Q1", y: 0 }]);
});

test("align_series counts a blank value as zero", () => {
  const [, b] = align_series([
    series("a", { x: "Q1", y: 1 }),
    series("b", { x: "Q1", y: "" }),
  ]);
...
test("align_series returns series that already share every x unchanged", () => {
  ...
  assert.deepEqual(align_series(given), given);
});
