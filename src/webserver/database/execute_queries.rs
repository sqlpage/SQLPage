...
use super::{Database, DatabasePool, DbItem, DbKind};
use sqlx::pool::PoolConnection;
use sqlx::{
    Column, ColumnIndex, Database as SqlxDatabase, Either, Encode, Executor, IntoArguments,
    Row as _, Type, ValueRef,
};
...
pub enum DbConnection {
    ...
    Mssql(PoolConnection<sqlx_sqlserver::Mssql>),
    Odbc(PoolConnection<sqlx_odbc::Odbc>),
}
...
async fn try_rollback_transaction<DB>(db_connection: &mut PoolConnection<DB>)
where
    DB: SqlxDatabase,
    for<'c> &'c mut DB::Connection: Executor<'c, Database = DB>,
{
    log::debug!("Attempting to rollback transaction");
    match (&mut **db_connection).execute("ROLLBACK").await {
...
async fn rollback_connection(connection: &mut DbConnection) {
    match connection {
        DbConnection::Sqlite(conn) => try_rollback_transaction(conn).await,
        DbConnection::Postgres(conn) => try_rollback_transaction(conn).await,
        DbConnection::MySql(conn) => try_rollback_transaction(conn).await,
        DbConnection::Mssql(conn) => try_rollback_transaction(conn).await,
        DbConnection::Odbc(conn) => try_rollback_transaction(conn).await,
    }
}
...
async fn fetch_optional_string(
    ...
    match connection {
        DbConnection::Sqlite(conn) => {
            fetch_optional_string_inner::<sqlx::Sqlite>(conn, query, query_span).await
        }
        DbConnection::Postgres(conn) => {
            fetch_optional_string_inner::<sqlx::Postgres>(conn, query, query_span).await
        }
        DbConnection::MySql(conn) => {
            fetch_optional_string_inner::<sqlx::MySql>(conn, query, query_span).await
        }
        DbConnection::Mssql(conn) => {
            fetch_optional_string_inner::<sqlx_sqlserver::Mssql>(conn, query, query_span).await
        }
        DbConnection::Odbc(conn) => {
            fetch_optional_string_inner::<sqlx_odbc::Odbc>(conn, query, query_span).await
        }
    }
}

async fn fetch_optional_string_inner<DB>(
    connection: &mut PoolConnection<DB>,
    query: &StatementWithParams<'_>,
    query_span: tracing::Span,
) -> sqlx::Result<Option<Option<String>>>
...
{
    (&mut **connection)
        .fetch_optional(bind_query::<DB>(query))
        .instrument(query_span)
        .await
        .map(|row| row.map(|row| row_to_string(&row)))
}
...
async fn execute_set_variable_query<'a>(
    ...
    let value = match fetch_optional_string(connection, &query, query_span.clone()).await {
        Ok(Some(value)) => {
        ...
        Err(e) => {
            query_metrics.add_duration(start_time.elapsed());
            rollback_connection(connection).await;
...
async fn take_connection<'a>(
    ...
    conn: &'a mut DbConn,
    request: &ExecutionContext,
) -> anyhow::Result<&'a mut DbConnection> {
    if let Some(c) = conn {
        return Ok(c);
    }
    ...
    let acquired = acquire_connection(&db.connection)
        .instrument(acquire_span)
        .await
        .with_context(|| {
            let db_name = db.info.kind.display_name();
...
async fn acquire_connection(pool: &DatabasePool) -> sqlx::Result<DbConnection> {
    match pool {
        DatabasePool::Sqlite(pool) => pool.acquire().await.map(DbConnection::Sqlite),
        DatabasePool::Postgres(pool) => pool.acquire().await.map(DbConnection::Postgres),
        DatabasePool::MySql(pool) => pool.acquire().await.map(DbConnection::MySql),
        DatabasePool::Mssql(pool) => pool.acquire().await.map(DbConnection::Mssql),
        DatabasePool::Odbc(pool) => pool.acquire().await.map(DbConnection::Odbc),
    }
}
...
async fn set_trace_context(connection: &mut DbConnection, db: &Database) {
    ...
    let result = match connection {
        DbConnection::Postgres(conn) => (&mut **conn)
            .execute(sqlx::AssertSqlSafe(sql.clone()))
            .await
            .map(|_| ()),
        DbConnection::MySql(conn) => (&mut **conn)
            .execute(sqlx::AssertSqlSafe(sql.clone()))
            .await
            .map(|_| ()),
        _ => return,
...
#[inline]
fn parse_single_sql_result<DB>(
    source_file: &Path,
    stmt: &StmtWithParams,
    res: sqlx::Result<Either<DB::QueryResult, DB::Row>>,
) -> DbItem
where
    DB: SqlxDatabase,
    DB::QueryResult: std::fmt::Debug,
...
async fn bind_parameters<'a>(
    ...
    Ok(StatementWithParams { sql, param_values })
}
...
fn bind_query<'q, DB>(
    statement: &'q StatementWithParams<'q>,
...
where
    ...
    DB::Arguments: IntoArguments<DB>,
{
    let mut query = sqlx::query::<DB>(sqlx::AssertSqlSafe(statement.sql));
    for value in &statement.param_values {
