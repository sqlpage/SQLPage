...
async fn execute_set_variable_query<'a>(
    ...
    let value = match connection
        .fetch_optional(query.sql, &query.arguments)
        .instrument(query_span.clone())
        .await
    {
        Ok(Some(row)) => {
...
async fn take_connection<'a>(
    ...
    match db.connection.acquire().instrument(acquire_span).await {
        ...
        Err(e) => {
            let db_name = db.connection.kind();
...
async fn set_trace_context(connection: &mut DbConnection, db: &Database) {
    ...
    if let Err(e) = connection.execute_command(sql.as_str(), &[]).await {
...
fn parse_single_sql_result(
    _source_file: &Path,
    _stmt: &StmtWithParams,
    res: DbStatementResult,
) -> DbItem {
    match res {
        DbStatementResult::Row(r) => {
            if log::log_enabled!(log::Level::Trace) {
                debug_row(&r);
            }
            DbItem::Row(super::sql_to_json::row_to_json(&r))
        }
        DbStatementResult::Finished => {
            let res = "finished";
...
fn debug_row(r: &super::driver::DbRow) {
    use std::fmt::Write;
...
async fn bind_parameters<'a>(
    ...
    let mut arguments = Vec::with_capacity(stmt.params.len());
    ...
    for (param_idx, param) in stmt.params.iter().enumerate() {
        ...
        arguments.push(DbParam::from(argument.map(Cow::into_owned)));
    }
