...
impl Database {
    pub async fn init(config: &AppConfig) -> anyhow::Result<Self> {
        ...
        log::debug!("Connecting to a {db_kind:?} database on {}", config.database_url);
        ...
        let connection = match db_kind {
            DbKind::Sqlite => {
                ...
                let pool = Self::create_pool_options::<sqlx::Sqlite>(config, db_kind)
    ...
    usize: ColumnIndex<DB::Row>,
{
    Box::pin(async move {
        ...
        let query_result = conn.fetch_optional(sqlx::AssertSqlSafe(sql.as_str())).await?;
...
fn database_url_with_password(config: &AppConfig) -> anyhow::Result<String> {
    ...
    url.set_password(Some(password))
        .map_err(|_| anyhow::anyhow!("Unable to set password in database URL"))?;
    Ok(url.to_string())
}
